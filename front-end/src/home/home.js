                            ...
                            {dailyWords.map((wordInfo, index) => {
                                return (
                                    <tr className='daily-puzzles-table-content-row' key={index}>
                                        ...
                                        <td className='daily-puzzles-table-content-cell'>{played && played.find(x => x.link === wordInfo.uuid) ? `${played.find(x => x.link === wordInfo.uuid).guesses} / 6` : null}</td>
                                        <td className='daily-puzzles-table-content-cell'>{played && played.find(x => x.link === wordInfo.uuid) ? `${played.find(x => x.link === wordInfo.uuid).win}` : null}</td>
                                    </tr>
                                )
                            })}
                        </tbody>
